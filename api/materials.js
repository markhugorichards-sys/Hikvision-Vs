 ...
 *                                        category, sector, persona, productKey }  (upload)
 *   PUT    /api/materials          -> { id, type, category, sector, persona,
...
function publicEntry(m) {
  return {
    ...
    category: m.category, sector: m.sector, persona: m.persona, productKey: m.productKey,
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const { filename, mime, dataBase64, type, category, sector, persona, productKey } = body || {};
    ...
    // ---- PUT: edit an existing material's tags (type/category/sector/persona/productKey) ----
    // The file blob itself is never touched here — only the index entry's metadata changes.
    if (req.method === 'PUT') {
      let body = req.body;
      if (typeof body === 'string') { try { body = JSON.parse(body); } catch { body = {}; } }
      const { id, type, category, sector, persona, productKey } = body || {};
